...
RSpec.describe Claims::ClaimValidator, type: :service do
  let(:valid_claim) { build(:claim, amount: 100.0, receipts: [Rack::Test::UploadedFile.new(Rails.root.join('spec/fixtures/receipt.jpg'), 'image/jpeg')]) }
  let(:invalid_claim) { build(:claim, amount: -1, receipts: [Rack::Test::UploadedFile.new(Rails.root.join('spec/fixtures/receipt.jpg'), 'image/jpeg')]) }
